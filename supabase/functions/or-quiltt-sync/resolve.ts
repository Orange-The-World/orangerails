 ...
 *
 * The prefix class is {1,8}. A Quiltt profile id is `p_`, so a single character
 * prefix is the common case rather than the edge one, and the class has to reach
 * down to one character to cover it. Over-redacting an ordinary word that happens
 * to look like an id is the safe direction to be wrong in.
 */
