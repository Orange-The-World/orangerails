    ...
    for (const conn of connections) {
      try {
        ...
        if (conn.provider_type === 'strike') {
          ...
          //
          // walletsByCurrency maps uppercase ISO currency codes (e.g. 'BTC', 'USD')
          // to the external_wallet_id for that currency, built from the source_wallets
          // rows selected above. drainStrikeQueue uses it to attribute each drained
          // transaction to the correct per-currency wallet.
          // Rows without a currency value (legacy connections) produce no entry and
          // fall back to null source_wallet_id inside queue.ts.
          const walletsByCurrency = new Map<string, string>(
            (sourceWallets ?? [])
              .filter((w: { external_wallet_id: string; is_synced: boolean; currency: string | null }) => w.currency)
              .map((w: { external_wallet_id: string; currency: string }) => [
