...
Deno.serve(wrapSentryHandler(async (req)=>{
  ...
  try {
    ...
    if (body.agent_name.length > 100) {
      return jsonResponse({
        error: 'agent_name too long (max 100 chars)'
      }, 400, cors);
    }
    ...
    if (error) {
      ...
      const status = error.message?.includes('Unauthorized') ? 401 : error.message?.includes('Too many') ? 429 : 400;
      return jsonResponse({
        error: error.message
      }, status, cors);
    }
