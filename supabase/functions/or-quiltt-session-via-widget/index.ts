  ...
  wrapSentryHandler(async (req: Request) => {
    ...
    try {
      ...
      if (platform.error || !platform.data) {
        console.error(
          "[or-quiltt-session-via-widget] platform lookup failed:",
      ...
      if (subUpsert.error || !subUpsert.data) {
        console.error(
          "[or-quiltt-session-via-widget] subaccount upsert failed:",
      ...
      if (mapLookup.error) {
        console.error(
          "[or-quiltt-session-via-widget] profile lookup failed:",
          mapLookup.error.message,
        );
      ...
      if (!quilttApiKey) {
        ...
        return jsonResponse(
          { error: "Quiltt integration not configured (QUILTT_API_KEY)" },
          503,
          cors,
        );
      }
      if (!connectorId) {
        ...
        return jsonResponse(
          { error: "Quiltt integration not configured (connector id)" },
          503,
          cors,
        );
      }
      ...
      if (!mintResp.ok) {
        ...
        console.error(
          `[or-quiltt-session-via-widget] Quiltt mint failed: ${mintResp.status} ${errText.slice(0, 300)}`,
        );
      ...
      if (!mapLookup.data) {
        const insertMap = await service.from("quiltt_profile_map").insert({
        ...
        if (insertMap.error) {
          console.error(
            "[or-quiltt-session-via-widget] profile map insert failed:",
            insertMap.error.message,
          );
      ...
      console.error(
        "[or-quiltt-session-via-widget] error:",
