...
interface XpubCredentials {
  ...
  xpub: string;
  /**
   * BIP44 gap limit. Not exposed in the credential form: the user never
   * needs to touch this and the default (20) is correct for virtually all
   * personal wallets. Kept as a parsed field so server-side callers can
   * still pass a custom value if needed (e.g. testing or high-volume wallets).
   */
  gap_limit?: number;
}
...
export const xpubAdapter: ProviderAdapter = {
  ...
  credentialFields: [
    ...
    // for virtually all personal wallets and the question confused users.
    // The server defaults to 20 in parseXpubCredentials when the field is
    // absent, so no server-side change is needed.
  ],
