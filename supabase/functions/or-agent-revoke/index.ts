...
Deno.serve(wrapSentryHandler(async (req)=>{
  ...
  try {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      return jsonResponse({
        error: 'Authorization: Bearer <user-jwt> required'
      }, 401, cors);
    }
    ...
    if (body.reason !== undefined && (typeof body.reason !== 'string' || body.reason.length > 500)) {
      return jsonResponse({
        error: 'reason must be a string ≤500 chars'
      }, 400, cors);
    }
    ...
    if (revokeErr) {
      ...
      const status = revokeErr.message?.includes('Unauthorized') ? 401 : revokeErr.message?.includes('Forbidden') ? 403 : revokeErr.message?.includes('not found') ? 404 : 400;
      return jsonResponse({
    ...
    if (!row?.agent_member_id) {
      return jsonResponse({
        error: 'Revoke returned no result'
      }, 500, cors);
    }
    ...
    if (row.shadow_user_id && !row.was_already_revoked) {
      ...
      const { error: deleteErr } = await admin.auth.admin.deleteUser(row.shadow_user_id);
      if (deleteErr) {
    ...
    return jsonResponse({
      ...
      wrapped_keys_deleted: row.wrapped_keys_deleted ?? 0,
