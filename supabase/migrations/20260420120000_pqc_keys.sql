-- ============================================================
-- Post-quantum key material for forthcoming role-scoped key wrapping.
-- ============================================================
-- Context: A future role-scoped-keys feature will wrap per-role data
-- keys for each authorized recipient. To keep that feature quantum-safe
-- from day one (and protect against harvest-now-decrypt-later attackers),
-- the recipient key material is a HYBRID of X25519 (RFC 7748) and
-- ML-KEM-768 (FIPS 203), and audit signatures use ML-DSA-65 (FIPS 204).
--
-- Primitives live in src/lib/pqc.ts. Orchestration in key-wrapping.ts
-- (KEM) and signatures.ts (signer). Vault lifecycle in pqc-lifecycle.ts.
--
-- This migration only lays out the storage columns and the
-- wrapped_data_keys table. No data is written until the app first runs
-- the PQC lifecycle against each existing vault.
--
-- Related docs: docs/OrangeRails-PQC.md (threat model, algorithm choice,
-- migration path to ML-KEM-1024 / ML-DSA-87).
...
-- Writes remain service-role only until the role-scoped-keys feature
-- lands. That feature will introduce explicit INSERT/DELETE policies
