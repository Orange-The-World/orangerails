...
-- ── opk_deferred_at: OPK-pending gate ───────────────────────────────
-- This column was applied out-of-band to dev before this migration was
-- updated. These statements own the change in migration history so that
-- repo and schema remain identical, and so the prod apply can be
-- verified against the file rather than reconstructed from memory.

-- 1) add the column (retirement_reason is already present on prod)
...
-- 2) rebuild the pending index to exclude OPK-deferred rows.
--    A partial-index predicate cannot be ALTERed in place: drop + recreate.
--    On dev this was a zero-row table so the lock window was negligible.
--    On prod the CTO applies this under the two-party gate; row count and
--    lock window are noted in the Delivery thread (DL-0421 / DL-0426).
