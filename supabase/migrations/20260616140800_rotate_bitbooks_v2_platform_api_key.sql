...
-- 20260424120000) lived only on Daenon's Vercel and was never recorded in
...
--
-- The new raw key was generated on bb-support 2026-06-16 by
...
-- shared with Daenon via the same channel for him to put on Vercel.
--
-- Cutover plan (coordinated to minimise prod auth window):
--   1. Daenon updates Vercel OR_PLATFORM_API_KEY to the new value on all
--      three scopes (Production, Preview, Development) and stages a
--      redeploy.
--   2. Daenon confirms ready.
--   3. THIS migration is merged to OR prod (CI applies via Mgmt API).
--   4. Daenon triggers the Vercel redeploy. V2 prod starts sending the
