...
--
-- TOLERATES 20260831193000 NOT HAVING APPLIED YET
--   Step 1 only drops a constraint by that exact name if one exists; on a
--   freshly rebuilt cluster where 20260831193000 has not run yet, there is
--   nothing to drop and step 1 is a no-op. Step 2 then adds the widened
--   constraint directly, so the end state is identical either way: both
--   migrations applied in file order, or this one applied alone.
