...
describe("forced differences, each one must go red", () => {
  ...
  it("buildFindings still classifies a relkind v row as a view (defensive; the live query never emits one, see OR-T2653)", () => {
    const rows = [
      ...BASE_ROWS,
...
describe("views are out of scope (OR-T2653)", () => {
  it("SCHEMA_SQL excludes relkind v and m, so a live view can never reach buildFindings as a finding on either side", async () => {
    const { SCHEMA_SQL } = await import("./check-types-drift.mjs");
    expect(SCHEMA_SQL).toContain("c.relkind in ('r','p')");
    expect(SCHEMA_SQL).not.toContain("'v'");
    expect(SCHEMA_SQL).not.toContain("'m'");
  });
});
