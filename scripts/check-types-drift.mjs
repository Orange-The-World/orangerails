 ...
 *
 * Views are OUT OF SCOPE (OR-T2653). parseTypesFile only reads the generated
 * Tables block, never the Views block, so a live view could never match
 * anything the parser produced and became a permanent missing-view finding
 * that no regeneration could clear. Rather than parse the Views block too,
 * views (relkind v and m) are dropped from the schema query below, so a view
 * can never appear as a finding on either side. If a real drift ever needs
 * catching in a view's own shape, that is new scope for a future change, not
 * a gap in this one.
 *
 * Environment:
...
/**
 * One row per column of every table and partitioned table in the public
 * schema. Views and materialized views (relkind v, m) are deliberately
 * excluded: see the OR-T2653 note above. attidentity and attgenerated count
 * as a default: both mean an insert may legitimately omit the column.
 */
