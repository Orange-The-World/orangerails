...
-- the routine name, including the space inside the quotes, so `short` became
-- "purgeaudit", a name that does not exist. The invocation side only
-- collapses whitespace, so it kept the space. The call regex built from
-- "purgeaudit" could never match "purge audit(", the body was never scanned,
-- and this file classified REVERSIBLE while it empties a table.
--
