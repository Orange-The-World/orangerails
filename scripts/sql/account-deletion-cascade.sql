...
--
-- WHY EVERY UNEXPECTED ERROR IS ALSO A DEV0323_FAIL, NAMING THE LEG. Before this
-- revision, an error the four legs below did not anticipate (most concretely: a
-- guard trigger on one of these tables refusing a write for a reason unrelated
-- to account deletion) propagated as a bare Postgres error with no DEV0323_
-- sentinel in it. The caller's classify() then reported it as UNKNOWN blaming
-- "an expired or rotated SUPABASE_ACCESS_TOKEN", which sends whoever reads the
-- CI log looking for a credential problem that does not exist, while the real
-- story, a guard silently blocking the cascade this check exists to prove, goes
-- unread. The EXCEPTION handler at the bottom of the DO block catches anything
-- that is not one of this file's own DEV0323_PASS/DEV0323_FAIL raises and
-- re-raises it as DEV0323_FAIL, naming the leg that was running and carrying
-- the original Postgres message (SQLERRM) verbatim.
--
...
DECLARE
  ...
  auth_after uuid;
  deltype "char";
  current_leg text := 'LEG A (the two foreign keys that make the deletion reachable at all)';
BEGIN
  ...
  current_leg := 'LEG B (deleting the account clears user_vault_meta)';
  ...
  current_leg := 'LEG C (deleting the customer clears customer_vault_meta)';
  ...
  current_leg := 'LEG D (deleting the account behind a customer clears that customer''s vault meta and nulls auth_user_id, keeping the customer row)';
...
EXCEPTION
  WHEN OTHERS THEN
    -- Our own deliberate PASS/FAIL raises above already carry a DEV0323_
    -- sentinel; let them through unchanged so the caller's classify() reads
    -- the specific leg that failed rather than this generic wrapper.
    IF SQLERRM LIKE 'DEV0323%' THEN
      RAISE;
    END IF;
    -- Anything else is a live Postgres error this file did not anticipate,
    -- most concretely a guard trigger on one of these tables refusing a
    -- write for a reason that has nothing to do with account deletion. That
    -- is still a real finding and it is reported as one: a FAIL naming which
    -- leg was running, with the original error attached, not an UNKNOWN that
    -- sends the reader chasing a token problem that does not exist.
    RAISE EXCEPTION
      'DEV0323_FAIL an unexpected error interrupted %, so the cascade could not be proven either way (this is a live Postgres error, not a token or connectivity problem): %',
      current_leg, SQLERRM;
END
$dev0323$;
