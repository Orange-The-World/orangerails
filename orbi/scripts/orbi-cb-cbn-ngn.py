#!/usr/bin/env python3
"""ORBI Tier 3  -  Central Bank of Nigeria (CBN) USD/NGN scraper.

CBN publishes the Nigerian Foreign Exchange Market (NFEM) daily rates via a
JSON endpoint backing /rates/ExchRateByCurrency.html. Per CBN's own footnote:
"NFEM rate is derived at Volume Weighted Average and stands as the official
exchange rate for the day."

Important context: Nigeria has historically had multiple coexisting rates  - 
the CBN official (NFEM weighted average), the I&E window, and the parallel
("black") market rate which has at times diverged from the official rate by
>50%. We use the NFEM weighted average as the authoritative CBN rate and
disclose the divergence in the notes column.

Endpoint: https://www.cbn.gov.ng/api/GetAllNFEM_RatesGRAPH
...
LOG = "/var/log/orbi/cb-cbn-ngn.log"
SOURCE_URL = "https://www.cbn.gov.ng/api/GetAllNFEM_RatesGRAPH"
SOURCE_AUTHORITY = "CBN"
SOURCE_CCY = "USD"
TARGET_CCY = "NGN"
USER_AGENT = "ORBI/1.0 (+https://orangethe.world; Bitcoin reference data; auditable)"
NOTES = "Official CBN rate; parallel market may diverge"
...
def _parse_ratedate(s):
    """CBN NFEM uses 'Month-DD-YYYY' (e.g. 'June-04-2026')."""
    mon, dd, yyyy = s.split("-")
...
def fetch_cbn():
    ...
    if not data or not isinstance(data, list):
        raise RuntimeError("CBN NFEM returned empty/invalid payload")

    # Pick the row with the most recent ratedate.
    def _key(row):
        return _parse_ratedate(row["ratedate"])
    latest = max(data, key=_key)
    published_date = _parse_ratedate(latest["ratedate"])
    rate_str = latest.get("weightedAvgRate") or latest.get("closingrate")
    if not rate_str:
        raise RuntimeError("CBN NFEM row missing weightedAvgRate and closingrate")
...
def upsert_exchange_rate(published_date_iso, usd_rate):
    ...
    sql = (
        ...
        f"VALUES ('USD', 'NGN', '{bucket_ts}', '1d', 'ORBI-D-authority', "
...
def main():
    log("=== CBN scrape start ===")
    raw, sha, published_date, usd_rate = fetch_cbn()
    log(f"fetched {SOURCE_URL}")
    log(f"  published_date={published_date}  USD/NGN={usd_rate:.4f}  sha256={sha[:16]}…")
    ...
    log(f"=== CBN scrape done (appended={appended}) ===")
