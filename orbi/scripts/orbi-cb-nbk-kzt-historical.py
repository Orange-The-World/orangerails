#!/usr/bin/env python3
"""ORBI Phase H  -  NBK USD/KZT historical backfill walker.

NBK exposes one date at a time via RSS XML:
  https://nationalbank.kz/rss/get_rates.cfm?fdate=DD.MM.YYYY
Walks backward day-by-day from yesterday to ~1999-12-01.
Idempotent; polite (1 req/sec).
"""
import hashlib
import json
import os
...
LOG = "/var/log/orbi/cb-nbk-kzt-historical.log"
...
PG = _pg_env()
RUN_ID = f"nbk-kzt-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}-{uuid.uuid4().hex[:8]}"
...
def fetch_day(d):
    ...
    url = SOURCE_URL_TMPL.format(dmy=dmy)
    ...
    # rss/item structure: <item><title>USD</title><description>148.46</description></item>
    for item in root.findall("item"):
        if (item.findtext("title") or "").strip().upper() == "USD":
            desc = (item.findtext("description") or "").strip()
            try:
                rate = float(desc)
            except ValueError:
                return None, sha, url, text
            # Quant col is per nominal 1 (NBK rates are per 1 USD).
...
def main():
    log(f"=== NBK historical backfill start run_id={RUN_ID} ===")
    ...
    while cur >= EARLIEST_DATE:
        ...
        cur -= timedelta(days=1)
        time.sleep(RATE_LIMIT_S)
    log(f"=== NBK historical backfill done  -  {total} writes ===")
    continuity_check()
