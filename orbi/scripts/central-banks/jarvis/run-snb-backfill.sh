#!/usr/bin/env bash
# SNB backfill wrapper — runs the Playwright runner against
# https://data.snb.ch/en/topics/ziredev/cube/devkua.
#
# Default host: bb-support (Playwright already installed for V2 E2E).
# If the SNB site ever Akamai-blocks bb-support's IP class, install this
# wrapper on jarvis (residential IP) — same pattern as run-rba-backfill.sh.
#
# Install:
#   sudo cp run-snb-backfill.sh /home/<user>/bin/run-snb-backfill.sh
#   sudo chmod 755 /home/<user>/bin/run-snb-backfill.sh
#
# Usage:
#   /home/<user>/bin/run-snb-backfill.sh <from YYYY-MM-DD> <to YYYY-MM-DD> [--dry-run]
#
# Example (dry-run, recent window):
#   /home/<user>/bin/run-snb-backfill.sh 2026-05-19 2026-05-26 --dry-run
#
# Example (full historical backfill — founder approval required):
#   /home/<user>/bin/run-snb-backfill.sh 1980-01-04 2026-05-26
set -euo pipefail
...
if [[ -z "$FROM" || -z "$TO" ]]; then
  echo "usage: run-snb-backfill.sh <from YYYY-MM-DD> <to YYYY-MM-DD> [--dry-run]" >&2
  exit 2
fi
...
ARGS=(scripts/central-banks/orchestrator.ts snb "$FROM" "$TO")
