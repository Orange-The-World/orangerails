...
describe("forwardHeaders", () => {
  it("re-injects the edge-set cf-connecting-ip under the trusted header and drops a caller-forged value under that same name", () => {
    ...
    src.set("x-real-ip", "10.0.0.2"); // caller-supplied, forgeable, must be stripped
    src.set("host", "api.orangerails.com");
    src.set("authorization", "Bearer abc123");
    ...
    expect(out.get("authorization")).toBe("Bearer abc123");
  });
  ...
  it("sets no trusted header when there is no edge-set cf-connecting-ip, and strips a caller-supplied x-forwarded-for", () => {
    const src = new Headers();
  ...
  it("strips a forged x-forwarded-for alongside a real cf-connecting-ip, forwarding the edge IP only under the trusted header (OR-T1444)", () => {
    const src = new Headers();
    src.set("cf-connecting-ip", "203.0.113.7"); // real edge-set value
    src.set("x-forwarded-for", "198.51.100.66"); // forged, caller-supplied
    src.set("x-real-ip", "198.51.100.67"); // forged, caller-supplied
