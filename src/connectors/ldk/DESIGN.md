...
The envelope is three columns (seal_version, sealed_iv, sealed_ct), not a
single sealed_blob: that is what the live table already has (verified
against the dev project, OR-T1721), and 3.3 makes the table the source of
truth here since the wiring PR ships zero DDL against it. There is no
updated_at column on channel_state.
