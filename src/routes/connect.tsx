...
interface ConnectSearch {
  // Optional widget session token minted by the
 ...
 *   unified-picker design review). Until then, integrating apps
    ...
    if (search.provider) {
      ...
      Promise.all([
        ...
        .then(([platformRes, manifestRes]) => {
          ...
          // This wired the picker click path for deep-linking; the
  ...
  async function handleConfirmPicks() {
    ...
    try {
      ...
      const result = await callLinkComplete({
        ...
        // Forward the widget session token the
