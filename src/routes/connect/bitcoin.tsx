 ...
 *
 * Fix (#760): derive bounce target from return_to when app_url is absent.
 * The picker forwards return_to on every arm but not app_url, making the
 * app_url branch unreachable for any consuming-app customer.
 *
 * DL-0680, #760
 */
...
// An unvalidated bounce target would be an open redirect.
...
function BitcoinConnectPage() {
  ...
  const handleLaunch = useCallback(async () => {
    ...
    // Prefer app_url; fall back to return_to (#760). The picker forwards
    // return_to on every arm but never app_url, so the consuming-app bounce
    // branch was unreachable when arriving through the hosted picker.
    const appUrl = params.get("app_url");
    const returnTo = params.get("return_to");
    const bounceTarget = appUrl ?? returnTo;

    if (bounceTarget) {
      // Bounce back to the consuming app (DL-0426). Validate origin.
      let origin: string | null = null;
      try {
        origin = new URL(bounceTarget).origin;
      } catch {
        origin = null;
      }
      ...
      console.warn(
        "[bitcoin] Refused bounce target with untrusted origin: " +
          (origin ?? "invalid URL") +
    ...
    // No bounce target and vault is locked: we cannot drive the stealth flow.
