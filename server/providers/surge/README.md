...
- `fixtures/positions.json` , one active position (0.005 BTC pledged, 2,000 USDC borrowed, 5.97% APR, LTV 40%) and one closed position
- `fixtures/summary.json` , aggregate matching the positions file
- `fixtures/accounting-state.json` , same data reshaped into `pledged_btc` + `line_of_credit` buckets
- `fixtures/events.json` , five events spanning loan_opened, collateral_added, borrowed, repaid, collateral_withdrawn
