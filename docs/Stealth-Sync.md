...
bug and a consumer-side bug that failed before ever posting INIT looked
...
doing the work. Do not build a consumer-side feature against these two
...
already have. This was a live bug in a consumer integration (fixed
2026-06-30); the fix belongs in the consuming app's close handler, not
in the widget.
...
provider credential OR holds on the user's behalf.
...
- **BDK WASM** (`@bitcoindevkit/bdk-wallet-web`): does not expose BIP 158 in its WASM target as of v0.3.0. Documented limitation: "Network access is limited to
