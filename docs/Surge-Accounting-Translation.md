...
Source spec: [Surge Partner API — Integration Plan](https://wiki.abascal.ca/doc/surge-Od9bf1GgpE).
...
For each Surge event, Orange Rails produces a draft journal entry (per the [OR-synced txs always land as DRAFT](https://wiki.abascal.ca) rule). Draft includes:
