...
- `sync-blink` Supabase function inlined the Blink GraphQL adapter. the internal gateway's Express adapter retired; `api.orangerails.com/sync/blink` proxy path removed from the Worker.
...
- `connect.orangerails.com` (Stealth Sync widget host), `blocks.orangerails.com` (BIP158 block source), `stealth.orangerails.com` (filter CDN) all wired through Caddy on the internal gateway and reachable end-to-end.
...
- the internal gateway Express server cut from the OR customer path. Blink GraphQL adapter inlined into the `sync-blink` Supabase function.
